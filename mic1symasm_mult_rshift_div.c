...
int main(int argc, char *argv[])
{
	...
	while(tok = yylex())
	{
		...
		case MULT:																	// When MULT appears in the .asm file, write proper instruction code to assembler
			if((tok = yylex()) != INTEG)
			{
			...
			str_6(yytext);
			fprintf(p1,"%d  1111111100%s\n", pc, cstr_6);							// Defining MULT command as "1111 1111 00mm mmmm"
			break;

		case RSHIFT:																// When RSHIFT appears in the .asm file, write proper instruction code to assembler
			if((tok = yylex()) != INTEG)
			{
			...
			str_6(yytext);
			fprintf(p1,"%d  1111111101%s\n",  pc, cstr_6);							// Defining RSHIFT command as "1111 1111 01xx ssss"
			break;

		case DIV:																	// When DIV appears in the .asm file, write proper instruction code to assembler
			fprintf(p1, "%d 1111111110000000\n", pc);								// Defining DIV instuction as "1111 1111 1000 0000" regardless of argument afterward
			break;
